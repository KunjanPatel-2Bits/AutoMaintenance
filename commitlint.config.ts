...
const Configuration: UserConfig = {
    ...
    parserPreset: {
        parserOpts: {
            headerPattern: /^(?<type>\w*)(?<scope>\([\w\d-]+\)?((?=:\s)|(?=!:\s)))?(?<breaking>!)?(?<subject>:\s.*)?|^(?<merge>Merge \w+)$/,
            headerCorrespondence: ['type', 'scope', 'breaking', 'dummy', 'subject', 'merge'],
        },
    },
    plugins: [
        {
